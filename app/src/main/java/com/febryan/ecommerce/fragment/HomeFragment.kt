...
import com.febryan.ecommerce.adapter.ProdukAdapter
...
//
//        rvBaju.adapter = AdapterProduk(arrBuku)
//        rvBaju.layoutManager = layoutManager2
    ...
    fun getDataProduk(){
        ApiConfig.instanceRetrofit.getProduk().enqueue(object : Callback<ResponseProduk>{
            override fun onResponse(call: Call<ResponseProduk>, response: Response<ResponseProduk>) {
                if (response.isSuccessful){
                    val responseProduk= response.body()
                    val produk = responseProduk?.produk
